...
public class PaymentCommandsListener {
	...
	@KafkaListener(topics = "${topics.payment.commands}", groupId = "payment-service-group")
	public void onPayment(@Header("x-event-type") String type,
		...
		try {
			switch (type) {
				case "payment.prepare" -> {
					...
					Map<String, Object> accepted = new java.util.HashMap<>();
					...
					log.info("[payment] payment.prepare.accepted published. orderId={}", dto.getOrderId());
					ack.acknowledge();
				}

				case "payment.cancel.requested" -> {
					try {
						...
						String canceled = tossPaymentClient.cancelPayment(customerId, payment.getPaymentKey(),
							"REQUEST_CANCEL");
				...
				default -> {
				log.debug("[Payment] ignore non-command type={}, key={}", type, key);
				ack.acknowledge();
				}
			}
		}catch (Exception e) {
		log.error("[Payment] command handle failed, type={}, key={}, body={}", type, key, body, e);
		}
	}
